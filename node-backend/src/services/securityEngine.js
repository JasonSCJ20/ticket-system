...
const NETWORK_FALLBACK_APP_NAME = 'Network Core Infrastructure';

// A bookkeeping row for findings generated by network-device automation
// (passive scan / IDS-IPS check), which need *an* ApplicationAsset to
// attach to but aren't really about any single web-reachable app. No real
// baseUrl exists to give it — "network infrastructure" isn't one HTTP
// endpoint — so enabled stays false to keep it out of Nuclei active
// scanning and downtime probing entirely, rather than inventing a
// placeholder URL that can never resolve.
export async function resolveNetworkFallbackAsset(ApplicationAsset) {
  const [app] = await ApplicationAsset.findOrCreate({
    where: { name: NETWORK_FALLBACK_APP_NAME },
    defaults: {
      baseUrl: null,
